#!/usr/bin/env python3
"""
Mass search with quality tiers: try high min_score first, then lower.
"""
import sys
import json
import random
...
def check_interior_viability(grid, across_entries, db):
    down_constraints = {}
    ...
    for r in range(8, SIZE):
        ...
        while c < SIZE:
            if grid[r][c] == '.':
                s = c
                while c < SIZE and grid[r][c] == '.': c += 1
                if c - s >= 3: runs_count += 1
                elif c - s > 0: return None
            else: c += 1
        if runs_count != a[r]: return None
...
def try_fill_at_score(min_score, time_budget):
    """Generate grids and try to fill them at the given min_score."""
    db = xword.WordDB(min_score=min_score)
    print(f"\n=== Trying min_score={min_score} ({db.total()} words, budget={time_budget}s) ===", flush=True)
    ...
    # Phase 1: Generate feasible grids
    gen_budget = min(time_budget * 0.3, 90)
    for seed_offset in range(1000):
        ...
        for sk in good_skeletons:
            ...
            for _ in range(200):
                ...
                if not check_interior_viability(grid, across, db):
                    continue
                ...
                if len(feasible_grids) % 10 == 0:
                    print(f"  {len(feasible_grids)} feasible grids...", flush=True)

    print(f"  {len(feasible_grids)} feasible grids found in {time.time()-t0:.1f}s")
    if not feasible_grids:
        return None

    feasible_grids.sort(key=lambda x: x[2])

    # Phase 2: Fill attempts
    for fi, (grid, across, max_len) in enumerate(feasible_grids):
        ...
        for attempt in range(40):
            ...
            if filler.dead:
                break
            ok = filler.solve(time_limit=20, jitter=15.0)
            if ok:
                ...
                print(f"\n*** SUCCESS at min_score={min_score}! grid#{fi} attempt {attempt} ***")
                filler.show()
                ...
                return puzzle
        if fi % 5 == 0:
            print(f"  grid {fi}/{len(feasible_grids)}...", flush=True)
...
def main():
    ...
    for min_score in [50, 40, 30, 20, 10, 0]:
        budget = 120 if min_score >= 30 else 180
        result = try_fill_at_score(min_score, budget)
    ...
    print("\nNo solution found at any quality level")
    sys.exit(1)
